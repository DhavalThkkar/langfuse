...
/**
 * Azure AD auth for Blob Storage when account keys are omitted. Uses
 * {@link DefaultAzureCredential} so AKS workloads can use Azure Workload Identity
 * (pod-injected AZURE_CLIENT_ID / AZURE_TENANT_ID / AZURE_FEDERATED_TOKEN_FILE),
 * managed identity, or local dev credentials—without embedding secrets in Langfuse env.
 */
function getAzureCredentialForBlobStorage(): DefaultAzureCredential {
  const managedIdentityClientId =
    env.LANGFUSE_AZURE_MANAGED_IDENTITY_CLIENT_ID?.trim() ||
  ...
  constructor(params: {
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      this.blobServiceClient = new BlobServiceClient(
        endpoint,
        getAzureCredentialForBlobStorage(),
      );
    }
